...
export class GameComponent {
  ...
  taskProgress(task: QuestTask): { current: number; total: number } | null {
    const c = task.condition;
    if (c?.type === 'gather' || c?.type === 'skill-action') {
      return { current: task.progress ?? 0, total: c.qty };
    }
    return null;
  }
      ...
      items: [
        {
          id: 'combat-level',
          label: 'Combat Level',
          pixelIcon: 'assets/icons/attack.png',
          badgeType: 'combat-level',
          component: CombatLevelComponent,
          children: [
            {
              id: 'attack',
              label: 'Attack',
            ...
            },
            {
              id: 'strength',
              label: 'Strength',
              pixelIcon: 'assets/icons/strength.png',
              badgeType: 'skill',
              navigateTo: 'combat-level',
            },
            {
              id: 'defence',
              label: 'Defence',
              pixelIcon: 'assets/icons/defence.png',
              badgeType: 'skill',
              navigateTo: 'combat-level',
            },
            {
              id: 'ranged',
              label: 'Ranged',
              pixelIcon: 'assets/icons/ranged.png',
              badgeType: 'skill',
              navigateTo: 'combat-level',
            },
            {
              id: 'hitpoints',
              label: 'Hitpoints',
              pixelIcon: 'assets/icons/hitpoints.png',
              badgeType: 'skill',
              navigateTo: 'combat-level',
            },
          ],
      ...
      items: [
        {
          ...
          unlockedByQuestStart: { questId: 'tutorial-island' },
          highlightedByQuestStep: { questId: 'tutorial-island', stepIndex: 0 },
        },
        {
          id: 'firemaking',
          label: 'Firemaking',
          ...
          unlockedByQuestStep: { questId: 'tutorial-island', stepIndex: 1 },
          highlightedByQuestStep: { questId: 'tutorial-island', stepIndex: 2 },
        },
        {
          id: 'fishing',
          label: 'Fishing',
          ...
          unlockedByQuestStep: { questId: 'tutorial-island', stepIndex: 2 },
          highlightedByQuestStep: { questId: 'tutorial-island', stepIndex: 3 },
        },
        {
          id: 'cooking',
          label: 'Cooking',
          ...
          unlockedByQuestStep: { questId: 'tutorial-island', stepIndex: 3 },
          highlightedByQuestStep: { questId: 'tutorial-island', stepIndex: 4 },
        },
        {
          id: 'mining',
          label: 'Mining',
          ...
          unlockedByQuestStep: { questId: 'tutorial-island', stepIndex: 4 },
          highlightedByQuestStep: { questId: 'tutorial-island', stepIndex: 5 },
        },
        {
          id: 'smithing',
          label: 'Smithing',
          ...
          unlockedByQuestStep: { questId: 'tutorial-island', stepIndex: 5 },
          highlightedByQuestStep: { questId: 'tutorial-island', stepIndex: 6 },
        },
        {
          id: 'crafting',
          label: 'Crafting',
          ...
          unlockedByQuestStep: { questId: 'tutorial-island', stepIndex: 6 },
          highlightedByQuestStep: { questId: 'tutorial-island', stepIndex: 7 },
        },
        {
          id: 'fletching',
          label: 'Fletching',
          ...
          unlockedByQuestStep: { questId: 'tutorial-island', stepIndex: 7 },
          highlightedByQuestStep: { questId: 'tutorial-island', stepIndex: 8 },
        },
        {
          id: 'crystallisation',
      ...
      items: [
        {
          id: 'settings',
          label: 'Settings',
          pixelIcon: 'assets/icons/settings.png',
          badgeType: 'none',
          locked: false,
  ...
  badgeFor(item: NavItem): string | null {
    ...
    switch (item.badgeType) {
      case 'skill':
        return `${p.skills[item.id as SkillId].level}/99`;
      case 'qp':
        return `QP: ${p.questPoints}`;
      case 'house-level':
        return `Level - ${p.houseLevel}`;
      case 'bank-space':
        return (p.bankCapacity - this.itemService.bankCount()).toString();
      case 'gold':
        return `${p.gold}G`;
      case 'location':
        return this.locationService.current().name;
      case 'combat-level':
  ...
  isItemLocked(item: NavItem): boolean {
    ...
    if (item.navigateTo) {
      const parent = this.sections
        .flatMap((s) => s.items)
        .find((i) => i.id === item.navigateTo);
      if (parent && this.isItemLocked(parent)) return true;
    }
    if (item.unlockedByQuestStart) {
      const quest = this.questService
        .quests()
        .find((q) => q.id === item.unlockedByQuestStart!.questId);
      if (!quest || quest.status === 'not-started') return true;
    }
    if (
      item.unlockedByQuestStep &&
      this.questService.getQuestStatus(item.unlockedByQuestStep.questId) !==
        'completed'
    ) {
