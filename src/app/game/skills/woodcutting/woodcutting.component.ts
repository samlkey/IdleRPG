...
export class WoodcuttingComponent {
  ...
  readonly trees = [
    { name: 'Normal Tree',  src: 'assets/objects/tree.png',            level: 1,  xp: 25,   time: 5,   critChance: 0.4, disabled: false },
  ...
  selectTree(name: string): void {
    ...
    if (current === name) {
      // Clicking the active tree stops it
      this.playerService.setMainActivity(null);
    } else {
      this.playerService.setMainActivity(name);
    }
  }
}
