...
const ANIMATIONS = {
  ...
  firemaking: {
    frames: Array.from(
      { length: 16 },
      (_, i) =>
        `assets/character/firemaking/firemaking${String(i + 1).padStart(2, '0')}.png`,
    ),
    ms: 80,
  },
} as const;
...
function makeTrack(key: CharacterAnimation, destroyRef: DestroyRef) {
  ...
  const timer = setInterval(
    () => index.update((i) => (i + 1) % frames.length),
    ms,
  );
...
export class CharacterAnimationService {
  ...
  readonly firemakingFrame = makeTrack('firemaking', this.destroyRef);
  constructor() {
